...
class MediaPreviewCard(GlassCard):
    ...
    def __init__(self, data, platform_color, original_url, on_download_video=None, on_download_audio=None, on_play=None, **kwargs):
        ...
        play_btn.bind(on_release=lambda *a: on_play and on_play(original_url, data.get("play_url")))
...
# ---------------------------------------------------------------------------
# Internal Video Player (ModalView) - IMPROVED VERSION
# ---------------------------------------------------------------------------
class VideoPlayerPopup(ModalView):
    def __init__(self, original_url, play_url=None, **kwargs):
        ...
        # Loading indicator
        self.loading_label = Label(text=ar("جاري التحميل..."), font_size="18sp", color=TEXT_MAIN, halign='center', valign='middle')
    ...
    def _load_video(self):
        ...
        if not video_url or not self._is_valid_url(video_url):
            # Try to extract a direct video URL using yt_dlp
            try:
                opts = {
                    "quiet": True,
                    "no_warnings": True,
    ...
    def _setup_video(self, video_url):
        ...
        self._layout.remove_widget(self.loading_label)
    ...
    def _on_video_loaded(self, *args):
        pass  # Video loaded successfully
