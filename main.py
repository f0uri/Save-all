...
class MediaPreviewCard(GlassCard):
    ...
    def __init__(self, data, platform_color, original_url, on_download_video=None, on_download_audio=None, on_play=None, **kwargs):
        ...
        play_btn.bind(on_release=lambda *a: on_play and on_play(original_url, data.get("play_url"), data.get("thumbnail")))
...
# ---------------------------------------------------------------------------
# Internal Video Player (ModalView) - ROBUST VERSION
#
# The old version showed a blank white box because Kivy's Video widget draws
# its playback surface *immediately* (white/blank) and only fills it with a
# real frame once decoding succeeds. Two things break that on Android:
#   1) The extracted URL is an HLS/DASH manifest (.m3u8) - ffpyplayer/
#      gstreamer often can't decode these reliably, so no frame ever arrives.
#   2) There was no timeout, so a stuck stream just sat there white forever.
# Fix: prefer a direct progressive mp4 URL, keep the thumbnail visible and
# the Video widget invisible until a decoded frame is *confirmed* (polling
# .texture, since on_load doesn't always fire), and fail loudly with a
# watchdog timer instead of hanging on a white screen.
# ---------------------------------------------------------------------------
class VideoPlayerPopup(ModalView):
    LOAD_TIMEOUT = 15  # seconds before we give up and tell the user

    def __init__(self, original_url, play_url=None, thumbnail=None, **kwargs):
        super().__init__(**kwargs)
        ...
        layout.bind(pos=self._upd_bg, size=self._upd_bg)

        # Stack: thumbnail behind, video on top (hidden until it has a real
        # frame), loading spinner/label on top of both. Nothing here is ever
        # plain white - worst case the user sees the thumbnail + spinner.
        self.media_stack = FloatLayout(size_hint=(1, 1))
        if thumbnail:
            self._thumb = AsyncImage(
                source=thumbnail, allow_stretch=True, keep_ratio=True,
                size_hint=(1, 1), pos_hint={'x': 0, 'y': 0}
            )
            self.media_stack.add_widget(self._thumb)
        with self.media_stack.canvas.before:
            Color(0.05, 0.05, 0.08, 1)
            self._stack_bg = Rectangle(pos=self.media_stack.pos, size=self.media_stack.size)
        self.media_stack.bind(pos=self._upd_stack_bg, size=self._upd_stack_bg)

        self.loading_label = Label(
            text=ar("جاري تحميل الفيديو..."), font_size="16sp", color=TEXT_MAIN,
            halign='center', valign='middle', pos_hint={'center_x': 0.5, 'center_y': 0.5}
        )
        self.media_stack.add_widget(self.loading_label)
        layout.add_widget(self.media_stack)

        self.add_widget(layout)
        self._layout = layout

        threading.Thread(target=self._load_video, daemon=True).start()
        self._watchdog_ev = Clock.schedule_once(self._on_timeout, self.LOAD_TIMEOUT)

    def _upd_bg(self, inst, val):
        self._bg_rect.pos = inst.pos
        self._bg_rect.size = inst.size

    def _upd_stack_bg(self, inst, val):
        self._stack_bg.pos = inst.pos
        self._stack_bg.size = inst.size
    ...
    @staticmethod
    def _pick_progressive_url(info):
        """Prefer a direct progressive (http, non-HLS/DASH) mp4 URL - this is
        the only kind of stream Kivy's video providers render reliably."""
        fmts = info.get('formats') or []
        candidates = [
            f for f in fmts
            if f.get('url')
        ...
        pool = mp4s or candidates
        if pool:
            pool.sort(key=lambda f: f.get('height') or 0)
            return pool[-1]['url']
        if fmts:
            return fmts[-1].get('url')
        return info.get('url')

    def _load_video(self):
        video_url = self.play_url
        if not video_url or not self._is_valid_url(video_url) or self._looks_like_hls(video_url):
            video_url = None
        if not video_url:
            try:
                opts = {
                ...
                with yt_dlp.YoutubeDL(opts) as ydl:
                    ...
                    if info:
                        video_url = self._pick_progressive_url(info)
            except Exception as e:
                Clock.schedule_once(lambda dt: self._show_error(str(e)[:200]), 0)
                return

        if not video_url:
            Clock.schedule_once(lambda dt: self._show_error(ar("تعذّر الحصول على رابط الفيديو")), 0)
            return

        Clock.schedule_once(lambda dt: self._setup_video(video_url), 0)

    def _setup_video(self, video_url):
        if self._ready:
            return  # popup already dismissed / timed out before this ran
        self.video = Video(
            source=video_url, state='play',
            options={'allow_stretch': True, 'eos': 'stop'}
        )
        self.video.opacity = 0  # stays hidden until a real frame decodes
        self.video.bind(on_load=self._on_video_loaded)
        self.video.bind(on_error=self._on_video_error)
        self.media_stack.add_widget(self.video, index=1)

        # on_load doesn't always fire reliably on Android providers, so also
        # poll for the first decoded texture - that's the real "not white
        # anymore" signal.
        self._poll_ev = Clock.schedule_interval(self._poll_ready, 0.1)

    def _poll_ready(self, dt):
        if self._ready:
            return False
        if self.video and self.video.texture is not None:
            self._reveal_video()
            return False
    ...
    def _reveal_video(self):
        ...
        if self._watchdog_ev:
            self._watchdog_ev.cancel()
            self._watchdog_ev = None
        Animation(opacity=0, duration=0.2).start(self.loading_label)
        Animation(opacity=1, duration=0.25).start(self.video)
        self._add_controls()

    def _on_video_loaded(self, *args):
        self._reveal_video()

    def _on_video_error(self, *args):
        self._show_error(ar("فشل تشغيل الفيديو، حاول التحميل بدلاً من المشاهدة"))

    def _on_timeout(self, dt):
        self._watchdog_ev = None
        if not self._ready:
            self._show_error(ar("استغرق تحميل الفيديو وقتًا طويلاً، جرّب التحميل مباشرة"))
    ...
    def _show_error(self, msg):
        ...
        try:
            ...
            if hasattr(app, 'toast'):
                app.toast.show_toast(msg, is_error=True)
        except Exception:
            pass
    ...
    def update_slider(self, dt):
        if self.video and self.video.duration and self.video.duration > 0:
